...
extern bool IsGtacoinDataOutput(const CTxOut& out);
extern bool IsGtacoinTxMine(const CTransaction& tx, const string &type);
extern std::string stringFromVch(const std::vector<unsigned char> &vch);
extern bool DecodeAndParseGtacoinTx(const CTransaction& tx, int& op, int& nOut, vector<vector<unsigned char> >& vvch);
enum {RECV=0, SEND=1};
static bool CreateGtacoinTransactionRecord(TransactionRecord& sub, int op, const vector<vector<unsigned char> > &vvchArgs, const CWalletTx &wtx, int type)
{
	COffer offer;
	CEscrow escrow;
	switch(op)
	...
	case OP_ALIAS_UPDATE:
		if(type == SEND)
			sub.type = (IsGtacoinTxMine(wtx, "alias")) ? TransactionRecord::AliasUpdate : TransactionRecord::AliasTransfer;	
		else if(type == RECV)
...
static bool CreateGtacoinTransactions(const CWallet *wallet, const CWalletTx& wtx, QList<TransactionRecord>& parts, const int64_t &nTime, const CAmount &nNet, const int type)
{
	if(wtx.nVersion != GetGtacoinTxVersion())
		return false;	
	uint256 hash = wtx.GetHash();
    vector<vector<unsigned char> > vvchArgs;
    int op, nOut;
	// there should only be one data carrying gtacoin output per transaction, but there may be more than 1 gtacoin utxo in a transaction
	...
	if(!DecodeAndParseGtacoinTx(wtx, op, nOut, vvchArgs))
		return false;

	TransactionRecord sub(hash, nTime);
	if(!CreateGtacoinTransactionRecord(sub, op, vvchArgs, wtx, type))
		return false;
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    if (nNet > 0 || wtx.IsCoinBase())
    {
        ...
        // Credit
        //
		// GTACOIN - this should be a received service
		if(!CreateGtacoinTransactions(wallet, wtx, parts, nTime, nNet, RECV))
		{
			BOOST_FOREACH(const CTxOut& txout, wtx.vout)
			{
				isminetype mine = wallet->IsMine(txout);
				if(mine)
				{
					...
					{
							// Received by Gtacoin Address
							sub.type = TransactionRecord::RecvWithAddress;
							// GTACOIN show alias in record
							CGtacoinAddress sysAddress = CGtacoinAddress(address);
							sysAddress = CGtacoinAddress(sysAddress.ToString());
    ...
    else
    {
        ...
        else if (fAllFromMe)
        {
            ...
            // Debit
            //
			// GTACOIN - this should be a new service you've created
			if(!CreateGtacoinTransactions(wallet, wtx, parts, nTime, nNet, SEND))
			{
