...
{
    CGtacoinAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered gtacoin address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CGtacoinAddress(rcp.address.toStdString()).Get());
...

    {
		...
		// GTACOIN pass in false to not sign
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal gtacoin:URI (gtacoin:123...?message=example)
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
    {
        ...
        if (!rcp.paymentRequest.IsInitialized())
        {
            ...
            CTxDestination dest = CGtacoinAddress(strAddress).Get();
...
// GTACOIN
static void NotifyGtacoinTransactionChanged(WalletModel *walletmodel, const CTransaction &tx, ChangeType status)
{
...
{
    QString strAddress = QString::fromStdString(CGtacoinAddress(address).ToString());
...
static void NotifyTransactionChanged(WalletModel *walletmodel, CWallet *wallet, const uint256 &hash, ChangeType status)
{
	...
	if(inWallet)
		NotifyGtacoinTransactionChanged(walletmodel, mi->second, status);
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
		...
		if(out.tx->nVersion == GetGtacoinTxVersion())
		{
			int op;
			vector<vector<unsigned char> > vvchArgs;
			// any gta tx thats not an alias payment shouldnt show up in listCoins (used by coin control)
			if (out.tx->vout.size() >= out.i && IsGtacoinScript(out.tx->vout[out.i].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
				continue;
		}
        ...
        if(/*!out.fSpendable || */!ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
		// GTACOIN
		CGtacoinAddress gtacoinAddress = CGtacoinAddress(address);
		gtacoinAddress = CGtacoinAddress(gtacoinAddress.ToString());
		QString qStrAddress = QString::fromStdString(gtacoinAddress.ToString());
		if(gtacoinAddress.isAlias)
			qStrAddress = QString::fromStdString(gtacoinAddress.aliasName);
...
{
    CTxDestination dest = CGtacoinAddress(sAddress).Get();
