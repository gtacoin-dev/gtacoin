...
extern int GetGtacoinDataOutput(const CTransaction& tx);
using namespace std;
...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    coinbaseTx.vout[0].scriptPubKey = scriptPubKeyIn;
	 // GTACOIN Compute final coinbase transaction. Add Gtacoin dynamic inflation based on service demand
...
void BlockAssembler::AddToBlock(CTxMemPool::txiter iter)
{
	...
	if(iter->GetTx().nVersion == GetGtacoinTxVersion())
	{
		nSysBlockTx++;
		if(nSysBlockTx >= 5)
		{
			int nOut = GetGtacoinDataOutput(iter->GetTx());
			if (nOut != -1)
