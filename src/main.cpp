...
#if defined(NDEBUG)
# error "Gtacoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Gtacoin Signed Message:\n";
...
// GTACOIN
bool CheckGtacoinInputs(const CTransaction& tx, const CCoinsViewCache& inputs, bool fJustCheck, int nHeight=0)
{
	...
	string errorMessage;
	if(tx.nVersion == GetGtacoinTxVersion())
	{
		bool good = true;
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
	...
	// GTACOIN
    if (fRequireStandard && tx.nVersion != GetGtacoinTxVersion() && tx.nVersion >= 2 && VersionBitsTipState(chainparams.GetConsensus(), Consensus::DEPLOYMENT_CSV) != THRESHOLD_ACTIVE) {
...

    {
		...
		// GTACOIN
        if (!CheckGtacoinInputs(tx, view, true))
			return false;
        // Remove conflicting transactions from the mempool
        BOOST_FOREACH(const CTxMemPool::txiter it, allConflicting)
        {
            LogPrint("mempool", "replacing tx %s with %s for %s GTA additional fees, %d delta bytes\n",
...
//
// GTACOIN check header auxpow proof of work
...
// GTACOIN: block header template's for auxpow integration
template<typename T>
...
// GTACOIN disconnect service related blocks
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    std::vector<int> prevheights;
	// GTACOIN inflate when high demand for services
    ...
    {
        const CTransaction &tx = block.vtx[i];
		// GTACOIN inflate and regenerate based on opreturn value set when creating the service(user)
		...
		if(tx.nVersion == GetGtacoinTxVersion())
		{
			...
			if(nSysBlockTx >= 5)
			{
				int nOut = GetGtacoinDataOutput(tx);
				if (nOut != -1)
        ...
        if (!tx.IsCoinBase())
        {
			...
			// GTACOIN
			if (!CheckGtacoinInputs(tx, view, fJustCheck, pindex->nHeight))
				return error("ConnectBlock(): CheckGtacoinInputs on %s failed",tx.GetHash().ToString());
            control.Add(vChecks);
        }
	...
	// GTACOIN Add dynamic inflation based on service demand
...
{
	// GTACOIN check auxpow first then fallback to normal check
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
	...
	// GTACOIN use GetBaseVersion() because of CBlockVersion
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (orphanTx.wit.IsNull() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/gtacoin/gtacoin2/issues/8279 for details.
                            assert(recentRejects);
