// Copyright (c) 2014-2015 The Gtacoin Core developers
...
// GTACOIN use aliases as addresses
...
namespace
{
class CGtacoinAddressVisitor : public boost::static_visitor<bool>
{
private:
    CGtacoinAddress* addr;
	// GTACOIN support old sys
	CChainParams::AddressType nSysVer;
public:
    CGtacoinAddressVisitor(CGtacoinAddress* addrIn) : addr(addrIn) {}
	CGtacoinAddressVisitor(CGtacoinAddress* addrIn, CChainParams::AddressType nSysVer) : nSysVer(nSysVer), addr(addrIn) {}
...
// GTACOIN aliases as addresses
CGtacoinAddress::CGtacoinAddress() {
	isAlias = false;
	aliasName = "";
	safeSearch = false;
	safetyLevel = 0;
...
// GTACOIN support old sys
CGtacoinAddress::CGtacoinAddress(const CTxDestination &dest, CChainParams::AddressType sysVer) { 
	isAlias = false;
	safeSearch = false;
	safetyLevel = 0;
	aliasName = "";
...
CGtacoinAddress::CGtacoinAddress(const std::string& strAddress) { 
	isAlias = false;
	aliasName = "";
...
CGtacoinAddress::CGtacoinAddress(const char* pszAddress) { 
	isAlias = false;
...
// GTACOIN support old sys
bool CGtacoinAddress::Set(const CKeyID& id, CChainParams::AddressType sysVer)
{   
...
bool CGtacoinAddress::Set(const CScriptID& id, CChainParams::AddressType sysVer)
{
...
// GTACOIN support old sys
bool CGtacoinAddress::Set(const CTxDestination& dest, CChainParams::AddressType sysVer)
{
    return boost::apply_visitor(CGtacoinAddressVisitor(this, sysVer), dest);
}

bool CGtacoinAddress::IsValid() const
{
    return IsValid(Params());
}

bool CGtacoinAddress::IsValid(const CChainParams& params) const
{
    bool fCorrectSize = vchData.size() == 20;
	// GTACOIN allow old GTACOIN address scheme
...
CTxDestination CGtacoinAddress::Get() const
{
    if (!IsValid())
...
bool CGtacoinAddress::GetKeyID(CKeyID& keyID) const
{
	// GTACOIN allow old GTACOIN address scheme
...
bool CGtacoinAddress::IsScript() const
{
...
void CGtacoinSecret::SetKey(const CKey& vchSecret)
{
...
bool CGtacoinSecret::IsValid() const
{
...
bool CGtacoinSecret::SetString(const char* pszSecret)
{
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CGtacoinSecret::SetString(const std::string& strSecret)
{
