#ifndef GTACOIN_TEST_TEST_GTACOIN_SERVICES_H
#define GTACOIN_TEST_TEST_GTACOIN_SERVICES_H

#include <stdio.h>
#include <univalue.h>
using namespace std;
/** Testing gtacoin services setup that configures a complete environment with 3 nodes.
 */
...
struct GtacoinTestingSetup {
    GtacoinTestingSetup();
    ~GtacoinTestingSetup();
};
struct BasicGtacoinTestingSetup {
    BasicGtacoinTestingSetup();
    ~BasicGtacoinTestingSetup();
};
struct GtacoinMainNetSetup {
	GtacoinMainNetSetup();
	~GtacoinMainNetSetup();
};
#endif
