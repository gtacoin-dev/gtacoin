...
const int GTACOIN_TX_VERSION = 0x7400;
...
bool IsGtacoinTxMine(const CTransaction& tx,const std::string &type);
...
bool DecodeAndParseGtacoinTx(const CTransaction& tx, int& op, int& nOut, std::vector<std::vector<unsigned char> >& vvch);
...
CAmount convertCurrencyCodeToGtacoin(const std::vector<unsigned char> &vchAliasPeg, const std::vector<unsigned char> &vchCurrencyCode, const double &nPrice, const unsigned int &nHeight, int &precision);
...
CAmount convertGtacoinToCurrencyCode(const std::vector<unsigned char> &vchAliasPeg, const std::vector<unsigned char> &vchCurrencyCode, const CAmount &nPrice, const unsigned int &nHeight, int &precision);
...
std::string GenerateGtacoinGuid();
bool IsAliasOp(int op);
...
bool GetGtacoinData(const CTransaction &tx, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash, int& nOut);
bool GetGtacoinData(const CScript &scriptPubKey, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash);
...
bool IsGtacoinScript(const CScript& scriptPubKey, int &op, std::vector<std::vector<unsigned char> > &vvchArgs);
...
void CleanupGtacoinServiceDatabases(int &servicesCleaned);
int aliasunspent(const std::vector<unsigned char> &vchAlias, COutPoint& outpoint);
bool IsMyAlias(const CAliasIndex& alias);
void GetAddress(const CAliasIndex &alias, CGtacoinAddress* address, const uint32_t nPaymentOption=1);
void GetAddress(const CAliasIndex &alias, CGtacoinAddress* address, CScript& script, const uint32_t nPaymentOption=1);
