...
extern bool IsGtacoinScript(const CScript& scriptPubKey, int &op, vector<vector<unsigned char> > &vvchArgs);
extern int GetGtacoinTxVersion();
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CGtacoinAddress(CScriptID(redeemScript)).ToString();
                                 ...
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    ...
    BOOST_FOREACH(const COutput &output, vCoins)
    {
		...
		// GTACOIN txs are unspendable unless input to another gtacoin tx (passed into createtransaction)
		if(pcoin->nVersion == GetGtacoinTxVersion())
		{
			int op;
			vector<vector<unsigned char> > vvchArgs;
			if (pcoin->vout.size() >= i && IsGtacoinScript(pcoin->vout[i].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
				continue;
		}
    ...
    BOOST_FOREACH(const COutPoint& outpoint, vPresetInputs)
    {
        ...
        if (it != mapWallet.end())
        {
			...
			if(pcoin->nVersion == GetGtacoinTxVersion())
			{
				...
				if (pcoin->vout.size() >= outpoint.n && IsGtacoinScript(pcoin->vout[outpoint.n].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
					continue;
			}
...
{
    CAmount nValue = 0;
	// GTACOIN: get output amount of input transactions for gtacoin service calls
	if(wtxInAlias != NULL)
	{
		if (nTxOutAlias < 0)
		{
			strFailReason = _("Can't determine type of alias input into gtacoin service transaction");
            return false;
		}
	}
	if(wtxInLinkAlias != NULL)
	{
		...
		{
			strFailReason = _("Can't determine type of linked alias input into gtacoin service transaction");
            return false;
		}
	}
	...
	// GTACOIN: set gtacoin tx version if its a gtacoin service call
	if(sysTx)
		txNew.nVersion = GetGtacoinTxVersion();
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH (const CRecipient& recipient, vecSend)
                {
					// GTACOIN pay to alias
					CRecipient myrecipient = recipient;
					CTxDestination payDest;
					int op;
					vector<vector<unsigned char> > vvchArgs;
					if (!myrecipient.scriptPubKey.IsUnspendable() && !IsGtacoinScript(myrecipient.scriptPubKey, op, vvchArgs)) {
						if (ExtractDestination(myrecipient.scriptPubKey, payDest)) 
						{
							CGtacoinAddress address = CGtacoinAddress(payDest);
							address = CGtacoinAddress(address.ToString());
							if(address.isAlias)
							{
				...
				// GTACOIN input credit from input tx
                ...
                CAmount nValueIn = 0;
				// GTACOIN add input credit to current coin selection
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-gtacoin-address
                    ...
                    if (coinControl && !boost::get<CNoDestination>(&coinControl->destChange))
					{
						...
						address = CGtacoinAddress(coinControl->destChange);
						address = CGtacoinAddress(address.ToString());
						if(!address.vchRedeemScript.empty())
                    ...
                    else
                    {
                       // Note: We use a new key here to keep it from being obvious which side is the change.
						...
						// GTACOIN pay to input destination as change
						CTxDestination payDest;
						...
						if (ExtractDestination(vecCoins[nLastIndex].first->vout[vecCoins[nLastIndex].second].scriptPubKey, payDest)) 
						{
							...
							address = CGtacoinAddress(payDest);
							address = CGtacoinAddress(address.ToString());
					...
					// GTACOIN change as a alias payment				
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CGtacoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CGtacoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CGtacoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CGtacoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CGtacoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CGtacoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CGtacoinAddress(dest).ToString(), key);
}
