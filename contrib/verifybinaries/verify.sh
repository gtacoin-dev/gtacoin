...
###   This script attempts to download the signature file SHA256SUMS.asc from gtacoin.org
...
WORKINGDIR="/tmp/gtacoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://gtacoin.org/bin/"
VERSIONPREFIX="gtacoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'gtacoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Gtacoin Core binary release signing key installed?"
   fi
