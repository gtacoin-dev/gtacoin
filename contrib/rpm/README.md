...
The RPM spec file provided here is for Gtacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://gtacoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Gtacoin git repository](https://github.com/gtacoin/gtacoin2) but are not part of
...
Currently only `Source10` is used, for the example `gtacoin.conf` file.
...
in the gtacoin git repository. At present only an SVG version of the Gtacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the gtacoin
...
needed when 0.12.1 is released, a proper fix is already in the Gtacoin git
master branch.
...
idea to build Gtacoin Core against a static version of that library compiled
...
The `gtacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `gtacoin-server` package. At this point, most RPM based
...
The `gtacoin-server` package should have SELinux support. How to properly do
...
Gtacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/gtacoin/). He has been
packaging Gtacoin for Fedora at least since 2012.
...
4. I renamed the `gtacoin` package that contains the Qt GUI to `gtacoin-core` as
that appears to be how the general population refers to it, in contrast to
`gtacoin-xt` or `gtacoin-classic`. I wanted to make sure the general population
